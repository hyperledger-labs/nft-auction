...
if [ "$CC_SKIP_PACKAGE_INSTALL" = "true" ]; then
  ...
  if [ "$CC_NAME" = "auctionchaincode" ]; then
    # peer0 of org1
    ...
    installChaincode 1 org2
	  errorln "Chaincode name should be 'auctionchaincode'"
  fi
fi
...
if [ "$CHANNEL_NAME" = "defaultchannel" ]; then
  ...
  approveForMyOrg 0 org2

  ## check whether the chaincode definition is ready to be committed
  ## expect all of them to have approved
  checkCommitReadiness 0 org1 "\"org1msp\": true" "\"org2msp\": true"
  checkCommitReadiness 1 org1 "\"org1msp\": true" "\"org2msp\": true"
